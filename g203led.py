...
def main(argv, send_status_text):
    if len(argv) < 2:
        help()
        sys.exit()

    args = argv + [None] * (4 - len(argv))
    ...
    if mode == 'solid':
        print(mode)
        set_led_solid(process_color(args[1]))
    elif mode == 'cycle':
        print(mode)
        set_led_cycle(process_rate(args[1]), process_brightness(args[2]))
    elif mode == 'breathe':
        print(mode)
        set_led_breathe(
            process_color(args[1]),
            process_rate(args[2]),
            process_brightness(args[3])
        )
    elif mode == 'intro':
        set_intro_effect(args[1])
    else:
...
def set_intro_effect(arg):
    ...
    if arg == 'on' or arg == '1' or arg is True:
        toggle = '01'
    elif arg == 'off' or arg == '0' or arg is False:
        toggle = '02'
    else:
